...
phases:
  build:
    commands:
      - PATH="$PATH:$CODEBUILD_SRC_DIR/sample-fraud-detection-and-prevention-fdp-agentic-platform/bin" && export PATH
      - if [ -d "$CODEBUILD_SRC_DIR/sample-fraud-detection-and-prevention-fdp-agentic-platform" ]; then mv $CODEBUILD_SRC_DIR/sample-fraud-detection-and-prevention-fdp-agentic-platform/ temp/; fi
      - git clone https://github.com/aws-samples/sample-fraud-detection-and-prevention-fdp-agentic-platform
      - if [ -d "temp" ]; then cp -R temp/ $CODEBUILD_SRC_DIR/sample-fraud-detection-and-prevention-fdp-agentic-platform/; rm -rf temp; fi
      - cd $CODEBUILD_SRC_DIR/sample-fraud-detection-and-prevention-fdp-agentic-platform/
...
cache:
  paths:
    - $CODEBUILD_SRC_DIR/sample-fraud-detection-and-prevention-fdp-agentic-platform
